...
public class GradeExtractApp
{
  ...
  private final String[] _courseIds;
  ...
  private GradeExtractApp( Properties appConfig )
  {
    ...
    //LOAD NEW PARAMETER, COURSE IDS / FOR AIEP
    String tempCourseIds = appConfig.getProperty( "app.courseIds", "" ).trim();
    if(tempCourseIds == null || tempCourseIds.length() == 0 || tempCourseIds.isEmpty()) {
      _courseIds = new String[0];
    } else {
      _courseIds = tempCourseIds.split(",");
    }
    
    int courseSize = _courseIds.length;
    for(int i=0; i < courseSize; i++) {
      _courseIds[i] = _courseIds[i].trim();
    }
